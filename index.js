...
async function registerRoutes(app2) {
  app2.get("/api/highscore", async (req, res) => {
    try {
  ...
  app2.post("/api/highscore", async (req, res) => {
    try {
...
app.use((req, res, next) => {
  ...
  res.json = function(body) {
    ...
    const logLine = `${req.method} ${path3} ${res.statusCode} in ${duration}ms${body ? ` :: ${JSON.stringify(body).slice(0, 79)}${JSON.stringify(body).length > 79 ? "\u2026" : ""}` : ""}`;
    log(logLine);
